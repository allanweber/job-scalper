...
_JOBCARDS_KEY = 'mosaic-provider-jobcards"]'
# Phrases that mark a Cloudflare interstitial rather than real results.
_CHALLENGE = re.compile(r"just a moment|cf-challenge|/cdn-cgi/challenge", re.IGNORECASE)
...
def parse_jobcards(page_html: str) -> list[dict]:
    ...
    if not page_html or is_challenge_page(page_html):
        return []
    key = page_html.find(_JOBCARDS_KEY)
    if key < 0:
        return []
    blob = _extract_json_object(page_html, key + len(_JOBCARDS_KEY))
    if not blob:
        return []
    try:
