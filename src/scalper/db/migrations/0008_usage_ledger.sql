-- Email-scoped usage ledger (abuse prevention).
--
...
-- the count. This ledger is keyed by email (not user_id) and is deliberately NOT
-- removed on account deletion, so signing up again with the same email carries
-- the used count forward and a user can't reset their monthly quota by deleting
